...
- **`OPENAI_LIKE_MODEL`** (Optional):
  ...
  - Set this in your shell profile (e.g., `~/.bashrc`, `~/.zshrc`) or an `.env` file.
- **`OPENAI_LIKE_DEPLOYMENT_ID`** (For Azure OpenAI):
  - The deployment ID for Azure OpenAI services.
  - Required when using Azure OpenAI endpoints.
- **`OPENAI_LIKE_API_VERSION`** (For Azure OpenAI):
  - The API version for Azure OpenAI services.
  - Required when using Azure OpenAI endpoints.
