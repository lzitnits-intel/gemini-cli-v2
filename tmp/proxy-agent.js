...
class ProxyAgent extends DispatcherBase {
  constructor (opts) {
    ...
    this.noProxyList = parseNoProxyInput(opts.noProxy || process.env.NO_PROXY)
  }
  ...
  dispatch (opts, handler) {
    ...
    if (shouldBypassProxy(originUrl, this.noProxyList)) {
      const directClient = new Client(originUrl, {
        connect: this[kConnectEndpoint],
        ...this[kRequestTls] ? { tls: this[kRequestTls] } : {}
      })
      return directClient.dispatch({ ...opts, headers }, handler)
    }

    if (!this.#shouldConnect(originUrl)) {
...
function parseNoProxyInput (input) {
  if (!input) return []
  return input.split(',').map(e => e.trim()).filter(Boolean)
}

function shouldBypassProxy (uri, noProxyList) {
  const url = new URL(uri)
  const hostname = url.hostname
  const port = url.port || defaultProtocolPort(url.protocol)

  if (!noProxyList || !Array.isArray(noProxyList)) return false

  for (const entry of noProxyList) {
    const [entryHost, entryPort] = entry.trim().split(':')

    if (entryHost === '*') return true

    if (entryPort && entryPort !== port.toString()) {
      continue
    }

    if (
      hostname === entryHost ||
      (entryHost.startsWith('.') && hostname.endsWith(entryHost)) ||
      (entryHost.startsWith('*.') && hostname.endsWith(entryHost.slice(1)))
    ) {
      return true
    }
  }
